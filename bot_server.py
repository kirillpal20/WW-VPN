...
# Секрет для внешнего "будильника" (cron-job.org), который раз в час
# дёргает /internal/check/<секрет>, чтобы запустить проверку серверов.
# Render free не даёт полноценных Cron Job — обходим так.
CHECKER_SECRET = os.environ.get("CHECKER_SECRET", "change-me-too")
...
@app.get("/internal/check/{secret}")
async def trigger_check(secret: str):
    """
    Дёргается внешним планировщиком (cron-job.org) раз в час.
    Запускает checker.run() в фоне и сразу отвечает — сам запрос
    от внешнего будильника не должен ждать 1-2 минуты проверки.
    """
    if secret != CHECKER_SECRET:
        raise HTTPException(status_code=403, detail="Неверный секрет")

    loop = asyncio.get_event_loop()
    loop.run_in_executor(None, checker.run)
